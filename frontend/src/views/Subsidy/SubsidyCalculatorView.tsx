...
export function SubsidyCalculatorView() {
  ...
  // Step 3: Vehicle & Variant Selection State
  ...
  const availableCatalogueVariants = selectedVehicleObj?.availableVariants || [];
  const selectedVariantObj = availableCatalogueVariants[selectedVariantIdx];
  ...
  // Variant-aware price & battery capacity calculations
  const calculatedExShowroom =
    vehicleSelectionMode === 'catalogue' && selectedVehicleObj
      ? selectedVariantObj
        ? selectedVariantObj.exShowroomPrice
  ...
  const calculatedBatteryKwh =
    vehicleSelectionMode === 'catalogue' && selectedVehicleObj
      ? selectedVariantObj
        ? selectedVariantObj.batteryCapacityKwh
        : selectedVehicleObj.batteryCapacityKwh
      : batteryCapacity;

  const activeCategory = vehicleSelectionMode === 'catalogue' && selectedVehicleObj ? selectedVehicleObj.category : vehicleCategory;

  // Policy Engine Subsidy Math (Delhi EV Policy 2026 Rules)
  const isEligiblePrice = activeCategory === '2W' ? calculatedExShowroom <= 225000 : calculatedExShowroom <= 3000000;
  const directStateSubsidy = isEligiblePrice
    ? activeCategory === '2W'
      ? Math.min(calculatedBatteryKwh * 10000, 30000)
      : activeCategory === '3W'
      ? 50000
      : 100000
    : 0;
                ...
                <motion.div initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                  ...
                  {vehicleSelectionMode === 'catalogue' ? (
                    <div className="space-y-5">
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-72 overflow-y-auto pr-1">
                          {MOCK_EMPANELLED_VEHICLES.map((veh) => {
                            ...
                            const variantCount = veh.availableVariants?.length || 0;
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex items-center justify-between text-xs text-slate-600 font-bold">
                                  ...
                                  <span className="text-emerald-700 font-extrabold">{veh.batteryCapacityKwh} kWh</span>
                                </div>
                                {variantCount > 0 && (
                                  <span className="text-[10px] text-slate-500 font-extrabold bg-slate-100 px-2 py-0.5 rounded-md inline-block w-fit">
                      ...
                      {/* Interactive Variant Selection for Selected EV */}
                      {selectedVehicleObj && availableCatalogueVariants.length > 0 && (
                        <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                          <div className="flex items-center justify-between">
                            <label className="text-xs font-black text-slate-700 uppercase tracking-wider">
                              Select Variant of {selectedVehicleObj.make} {selectedVehicleObj.model}
                            </label>
                            <span className="text-xs font-bold text-emerald-700">
                              Active: {selectedVariantObj?.name || selectedVehicleObj.variant}
                            </span>
                          </div>

                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {availableCatalogueVariants.map((v, idx) => {
                              const isVariantSelected = selectedVariantIdx === idx;
                              return (
                                <button
                                  key={v.id || idx}
                                  type="button"
                                ...
                                >
                                  ...
                                  <div className="text-[11px] opacity-90 font-medium">
                                    {v.batteryCapacityKwh} kWh • {v.claimedRangeKm} km • ₹{(v.exShowroomPrice / 100000).toFixed(2)} Lakh
                                  </div>
                                </button>
                              );
                            })}
