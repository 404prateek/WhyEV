...
export function DashboardOverview() {
  ...
  const [liveDash, setLiveDash] = React.useState<any>(null);

  React.useEffect(() => {
    let isMounted = true;
    import('@/lib/api').then(({ userApi }) => {
      userApi.getDashboardData().then((data) => {
        if (isMounted && data) {
          setLiveDash(data);
        }
      }).catch(() => {});
  ...
  const primaryApp = liveDash?.subsidy_applications?.[0];
  const activeDaysRemaining = primaryApp?.days_remaining ?? application.daysRemaining;
  const activeTotalBenefit = primaryApp?.total_benefit ?? application.totalBenefit;
  const activeCalculatedSubsidy = primaryApp?.calculated_subsidy ?? application.calculatedSubsidy;
  const activeScrappageBonus = primaryApp?.scrappage_bonus ?? application.scrappageBonus;
  const activeTaxWaiver = primaryApp?.tax_waiver_estimated ?? application.taxWaiverEstimated;
  const activeVehicleModelName = primaryApp?.vehicle_model_name ?? application.vehicleModelName;
              ...
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-extrabold bg-amber-300 text-amber-950 flex items-center gap-1.5">
                    ...
                    Day {30 - activeDaysRemaining} of 30 · {activeDaysRemaining} Days Left
                  </span>
                </div>
                <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
                  {activeDaysRemaining} Days Left to Claim {formatINR(activeTotalBenefit)} Subsidy
                </h2>
          ...
          <div id="three-card-grid" className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="p-7 rounded-3xl bg-white border border-slate-200/90 shadow-sm space-y-5 flex flex-col justify-between hover:shadow-md transition-all">
              <div className="space-y-4">
                ...
                <h3 className="text-base font-bold text-slate-900">{activeVehicleModelName}</h3>
                ...
                <div className="space-y-2.5 text-xs text-slate-600 font-medium bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-extrabold text-slate-900 text-sm">{formatINR(activeCalculatedSubsidy)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Scrappage Bonus:</span>
                    <span className="font-extrabold text-slate-900 text-sm">{formatINR(activeScrappageBonus)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Road Tax Waiver:</span>
                    <span className="font-extrabold text-slate-900 text-sm">~{formatINR(activeTaxWaiver)}</span>
                  </div>
                </div>
              </div>
