...
export function CitySelectorModal() {
  const { isCityModalOpen, closeCityModal, activeCityId, selectCity, isAutoDetecting, detectLocationGps } = useCityStore();
        ...
        >
          ...
          <div className="text-center sm:text-left space-y-2">
            <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
              Where are you shopping for an EV right now?
            </h2>
          </div>
                  ...
                  >
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-base font-black text-slate-900">{city.name}</span>
                          {isSelected && <CheckCircle2 className="w-4.5 h-4.5 text-emerald-600 shrink-0" />}
                        </div>
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 text-[10px] font-extrabold border border-slate-200 shrink-0">
                    ...
                    <div className="pt-2 border-t border-slate-100 flex flex-col gap-1 text-[11px] font-semibold text-slate-600">
                      <div className="flex items-center gap-1.5 text-emerald-800 font-bold">
                        ...
                        <span>Policy Active Until: {city.policyActiveUntil}</span>
                      </div>
                      <div className="flex items-center gap-1.5 text-slate-500">
                        <Store className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                        <span>{city.dealersCount}</span>
                      </div>
                    </div>
                  </button>
                );
