...
export function SubsidyCalculator() {
  ...
  const { requestPermission, isAuthenticated, openAuthModal } = useAuthStore();
        ...
        <div className="p-8 sm:p-10 rounded-3xl bg-white border border-slate-200/90 space-y-8 shadow-xl flex flex-col justify-between">
          <div className="space-y-6">
            ...
            {calculated && result && !loading && (
              <div className="space-y-6">
                ...
                {/* ── Post-Result Google Auth Nudge (shown only when not signed in) ── */}
                {!isAuthenticated && (
                  <div className="p-4 rounded-2xl bg-gradient-to-br from-emerald-50 to-slate-50 border border-emerald-200 flex flex-col sm:flex-row items-center gap-4">
                    <div className="flex-1 text-left">
                      <div className="text-xs font-extrabold text-slate-900 mb-0.5">Save & track your subsidy result</div>
                      <div className="text-[11px] text-slate-500 leading-relaxed">
                        Sign in to save this report, get deadline reminders, and generate your PDF certificate.
                      </div>
                    </div>
                    <button
                      onClick={() => openAuthModal()}
                      className="shrink-0 flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs transition-all shadow-sm cursor-pointer"
                    >
          ...
          <div className="pt-4">
            ...
            >
              ...
              <span>
                {!isAuthenticated && calculated && result?.eligible
                  ? 'Sign in to Get PDF Certificate'
                  : 'Generate Official PDF Eligibility Certificate'}
              </span>
            </button>
          </div>
        </div>
