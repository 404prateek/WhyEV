...
export function DocumentUploadSection() {
  ...
  const handleFileUpload = async (docId: string, file: File) => {
    ...
    // Clear previous error and set uploading state
    ...
    try {
      ...
      if (docId === 'rc' && !isPdf) {
        // Run OCR Vision Extraction on RC Image
      ...
      // Default upload pipeline
      await subsidyApi.uploadClaimDocuments(formData);
      ...
      // Fallback: set to uploaded state gracefully
      setDocStates((prev) => ({
        ...prev,
        [docId]: {
          status: 'uploaded',
