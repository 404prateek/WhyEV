...
export function TestDriveModal({ dealer, onClose }: TestDriveModalProps) {
  ...
  const handleFinalConfirm = async () => {
    ...
    // Convert 12-hour slot to ISO datetime for the backend
    const timeMap: Record<string, string> = {
      '10:00 AM': '10:00', '11:30 AM': '11:30', '02:00 PM': '14:00', '04:30 PM': '16:30',
    };
    const scheduledAt = `${preferredDate}T${timeMap[preferredTime] ?? '10:00'}:00`;
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                ...
                <button
                  ...
                  className="w-full py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs transition-all shadow-md shadow-emerald-600/20 cursor-pointer disabled:opacity-60"
                >
                  {isSubmitting ? 'Booking…' : 'Confirm & Continue'}
                </button>
              </div>
            </motion.div>
          )}
